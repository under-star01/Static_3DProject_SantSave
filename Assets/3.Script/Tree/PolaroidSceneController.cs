...
/// - 이후 TAB으로만 열고/닫기 가능 (첫 상호작용 전에는 TAB 입력 무시)
...
/// - PlayerInput     : 플레이어 입력 시스템 컨트롤 (Enable/Disable 제어 대상)
/// </summary>
public class PolaroidSceneController : MonoBehaviour
{
    ...
    [SerializeField] private GameObject dimmer;          // Dimmer (검은 배경)
    [SerializeField] private PolaroidBinder binder;      // CardRoot에 붙은 바인더
    ...
    // 추가: 플레이어 입력 시스템 참조
    [Header("Player Input")]
    [SerializeField] private PlayerInput playerInput; // 플레이어의 입력 컨트롤러 (Enable/Disable 제어용)
    ...
    private void OnEnable()
    {
        ...
        // 플레이어 입력 시스템 활성화 여부는 기본적으로 게임 시작 시 활성화 상태
        // 필요 시 초기 상태를 여기서 강제 조정 가능
        // 예: playerInput?.Enable();
    }
    ...
    private void CloseAndUnfreeze()
    {
        SetOpen(false);
        FreezeTime(false);
        // 닫힐 때는 UI를 비우고 바인더를 초기화하는 것도 좋습니다.
        if (binder != null)
        {
            binder.Clear();
        }
    ...
    // 새로운 API 추가: 플레이어 입력 활성화/비활성화 제어용 래퍼
